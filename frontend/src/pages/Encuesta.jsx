                ...
                <div className="flex flex-col gap-2">
                  {encuesta?.profesionales?.map(p => (
                    ...
                    >
                      {p.foto ? (
                        <img 
                          src={p.foto?.replace('/upload/', '/upload/w_200,h_200,c_fill,g_face,q_auto/')} 
                          alt={p.nombre} 
                          className={`w-20 h-20 rounded-full object-cover object-top border-4 shadow-md ${form.profesional_id === p.id ? 'border-green-600' : 'border-gray-200'}`} 
                        />
                      ) : (
