            ...
            <div className="overflow-y-auto p-6 flex flex-col gap-5">
              {cargandoHistorial ? (
                ...
                [
                  ...
                  { label: '🔬 Procedimientos', items: historial.procedimientos, render: p => (<><span className="text-gray-700">{p.nombre}</span>{rol !== 'matrona' && <span className="text-gray-400 text-xs">${Number(p.monto).toLocaleString('es-CL')}</span>}<span className="text-xs text-gray-400">{p.fecha?.slice(0,10)}</span></>) },
