    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4" onClick={onCerrar}>
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
        ...
        <p className="text-sm text-yellow-700 bg-yellow-50 rounded-lg px-3 py-2 mb-4">
          Completa los datos antes de iniciar la atención.
        </p>
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={guardar} className="flex-1 bg-green-700 text-white py-2 rounded-lg hover:bg-green-800 font-medium">
            Guardar e iniciar
          </button>
          <button onClick={onCerrar} className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-lg hover:bg-gray-200 font-medium">
            Cancelar
          </button>
        </div>
      </div>
...
export default function InicioMatrona({ usuario }) {
  ...
  const cargar = async () => {
    ...
    const citasHoy = c.data.filter(ci => ci.fecha_hora?.slice(0, 10) === hoyStr)
  ...
  const iniciarAtencion = async cita => {
    ...
    if (necesitaCompletar(paciente)) {
      ...
      await cambiarEstado(cita.id, 'en_atencion')
  ...
  const finalizarAtencion = async () => {
    if (citaAtendiendo) {
      await cambiarEstado(citaAtendiendo.id, 'realizada')
    }
    ...
    return (
      <div>
        <div className="flex items-center gap-3 mb-4">
          <button
            onClick={() => { setPacienteAtendiendo(null); setCitaAtendiendo(null); cargar() }}
    ...
    <div>
      {modalCompletar && (
        <ModalCompletarPaciente
          ...
          onConfirmar={async () => {
            const p = await axios.get(`${API_PAC}/${modalCompletar.paciente.id}`)
            await cambiarEstado(modalCompletar.cita.id, 'en_atencion')
            setModalCompletar(null)
            setPacienteAtendiendo(p.data)
                  ...
                  return (
                    <div key={c.id} className={`flex items-center gap-4 p-4 rounded-xl border ${
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {necesitaCompletar(paciente) && (
                          <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full mt-1 inline-block">Datos incompletos</span>
                        )}
                      </div>

                      {/* Estado y acciones */}
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className={`text-xs px-2 py-1 rounded-full font-medium ${estadoColor[c.estado]}`}>
                          {c.estado === 'en_atencion' ? 'En atención' : c.estado}
                        </span>
                        {(c.estado === 'confirmada' || c.estado === 'pendiente') && (
                          <button
                            onClick={() => iniciarAtencion(c)}
                            className="bg-purple-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-purple-700 transition-colors whitespace-nowrap"
                          >
                            Iniciar atención
                          </button>
                        )}
                        {c.estado === 'en_atencion' && (
                          <button
                            onClick={() => { setPacienteAtendiendo(getPaciente(c.paciente_id)); setCitaAtendiendo(c) }}
                            className="bg-purple-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-purple-700 transition-colors whitespace-nowrap"
                          >
                            Continuar
                          </button>
