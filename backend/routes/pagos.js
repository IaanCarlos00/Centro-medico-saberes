...
router.post('/', async (req, res) => {
  const { paciente_id, cita_id, monto, metodo, estado, notas } = req.body
  try {
    const result = await pool.query(
      'INSERT INTO pago (paciente_id, cita_id, monto, metodo, estado, notas) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *',
      [paciente_id, cita_id || null, monto, metodo, estado || 'pagado', notas || null]
    )
...
router.put('/:id', async (req, res) => {
  const { monto, metodo, estado, notas } = req.body
  try {
    const result = await pool.query(
      'UPDATE pago SET monto=$1, metodo=$2, estado=$3, notas=$4 WHERE id=$5 RETURNING *',
      [monto, metodo, estado, notas || null, req.params.id]
    )
