...
router.put('/:id', async (req, res) => {
  const { monto, metodo, estado, notas, numero_bono, estado_bono, estado_boleta, profesional_id, fecha, notas_abono } = req.body
  try {
    const result = await pool.query(
      'UPDATE pago SET monto=$1, metodo=$2, estado=$3, notas=$4, numero_bono=$5, estado_bono=$6, estado_boleta=$7, profesional_id=$8, fecha=$9, notas_abono=$10 WHERE id=$11 RETURNING *',
      [
        monto, metodo, estado || 'pendiente', notas || null, numero_bono || null,
        metodo === 'fonasa' ? (estado_bono || 'pendiente') : null,
        (metodo === 'efectivo' || metodo === 'transferencia') ? (estado_boleta || 'pendiente') : null,
        profesional_id || null, fecha || null, notas_abono || null, req.params.id
      ]
    )
